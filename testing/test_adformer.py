...
from eeg_dataset_adformer import create_adformer_dataloader, create_adformer_dataset
...
def evaluate_adformer_model(model, dataloader, device, class_names=["A", "C", "F"]):
    ...
    with torch.no_grad():
        for inputs_dict, labels in tqdm(dataloader, desc="Evaluating"):
            ...
            adformer_inputs, _ = dataloader.dataset.get_batch_for_adformer(
                (inputs_dict, labels)
            )
...
def test_adformer_model(data_type, model_path):
    ...
    try:
        ...
        model.to(device)
        print(
            f"ADformer model loaded from {model_path} with {count_parameters(model):,} parameters"
        )
    ...
    with open(results_file_txt, "w") as f:
        f.write(
            f"ADFORMER EVALUATION RESULTS: {data_type.upper()}\n\nModel: {model_path}\nSamples: {len(test_dataset)}\n"
    ...
    if not os.path.exists(model_path):
        for root, _, files in os.walk("."):
            for file in files:
                if (
                    file.endswith(".pth")
                    and "best_model_overall" in file
                    and "adformer" in root.lower()
                ):
