...
from eeg_adformer import count_parameters, create_adformer_eeg_model
from eeg_dataset_adformer import (
    create_vanilla_adformer_dataloader,
    create_vanilla_adformer_dataset,
)
...
def calculate_dataset_stats(dataset, num_samples=100):
    ...
    all_data = np.concatenate(
        all_data, axis=0
    )  # Shape: [num_samples*seq_len, channels]
...
def main():
    ...
    for fold, (train_idx, val_idx) in enumerate(fold_indices):
        ...
        train_stats_mean, train_stats_std = log_data_stats(
            train_dataset, "Training", log_message
        )
        valid_stats_mean, valid_stats_std = log_data_stats(
            valid_dataset, "Validation", log_message
        )
        ...
        if len(train_dataset) > 0 and len(valid_dataset) > 0:
            plot_sample_eeg(
                ...
                0,
                "Training Sample",
                f"{base_dir}/plots/train_sample_fold{fold+1}.png",
            )
            plot_sample_eeg(
                valid_dataset,
                0,
                "Validation Sample",
                f"{base_dir}/plots/valid_sample_fold{fold+1}.png",
            )

        # Compare distributions
        log_message(
            f"Mean difference between train and valid: {np.mean(np.abs(train_stats_mean - valid_stats_std))}"
        )
        log_message(
            f"Std difference between train and valid: {np.mean(np.abs(train_stats_std - valid_stats_std))}"
        )

        # Log validation class distribution
        valid_labels = [valid_dataset.labels[i] for i in range(len(valid_dataset))]
        class_counts = {
            0: valid_labels.count(0),
            1: valid_labels.count(1),
            2: valid_labels.count(2),
        }
        log_message(
            f"Validation class distribution for fold {fold + 1}: A: {class_counts[0]}, C: {class_counts[1]}, F: {class_counts[2]}"
        )
        ...
        log_message(
            f"Using Simplified ADformer EEG model with temporal patches: {patch_len_list}, channel dimensions: {up_dim_list}"
        )
        ...
        if len(train_dataloader) > 0:
            ...
            with torch.no_grad():
                ...
                log_message(
                    f"Initial model outputs - min: {sample_outputs.min().item()}, max: {sample_outputs.max().item()}"
                )
                log_message(
                    f"Initial model outputs - mean: {sample_outputs.mean().item()}, std: {sample_outputs.std().item()}"
                )
                initial_pred = sample_outputs.argmax(dim=1)
                log_message(
                    f"Initial predictions distribution: {torch.bincount(initial_pred, minlength=num_classes)}"
                )
        ...
        for epoch in range(start_epoch if fold == start_fold else 0, epochs):
            ...
            for batch_idx, (inputs_dict, labels) in enumerate(train_dataloader):
                ...
                adformer_inputs, _ = train_dataset.get_batch_for_adformer(
                    (inputs_dict, labels)
                )
                x_enc = adformer_inputs["x_enc"].to(device, non_blocking=True)
                x_mark_enc = adformer_inputs["x_mark_enc"].to(device, non_blocking=True)
                adformer_labels = adformer_inputs["labels"].to(
                ...
                train_loss += (
                    ce_loss.item() * accumulation_steps
                )  # Log only the CE loss

            epoch_train_loss = train_loss / len(train_dataloader)

            # Calculate training metrics
            train_precision, train_recall, train_f1, train_acc, _ = compute_metrics(
            ...
            with torch.no_grad():
                for inputs_dict, labels in valid_dataloader:
                    adformer_inputs, _ = valid_dataset.get_batch_for_adformer(
                        (inputs_dict, labels)
                    )
                    x_enc = adformer_inputs["x_enc"].to(device, non_blocking=True)
                    x_mark_enc = adformer_inputs["x_mark_enc"].to(
                        device, non_blocking=True
                    )
                    adformer_labels = adformer_inputs["labels"].to(
                        device, non_blocking=True
            ...
            precision, recall, f1, acc, conf_matrix = compute_metrics(
            ...
            if accuracy > best_fold_accuracy:
                ...
                log_message(
                    f"New best model for fold {fold+1} with accuracy: {accuracy:.2f}%"
                )
                if accuracy > best_accuracy:
                    ...
                    log_message(
                        f"New best overall model with accuracy: {accuracy:.2f}%"
                    )
                patience_counter = 0
            else:
                patience_counter += 1
            ...
            if len(epoch_valid_losses) >= 5:
                if all(
                    epoch_valid_losses[-1] > epoch_valid_losses[-i - 1]
                    for i in range(1, min(5, len(epoch_valid_losses)))
                ):
                    log_message(
                        f"Validation loss increasing for consecutive epochs. Early stopping."
                    )
                    break

            if patience_counter >= 10:  # Early stopping based on accuracy
                log_message(
                    f"No improvement in accuracy for 10 epochs. Early stopping."
                )
                break
