...
def get_latest_checkpoint(checkpoint_dir):
   ...
   return checkpoint_info[-1]  # Return the latest checkpoint info
...
if latest_checkpoint:
   ...
   log_message(f"Found checkpoint: Fold {fold_num}, Epoch {epoch_num}")
   ...
   log_message(f"Resuming training from fold {start_fold + 1}, epoch {start_epoch + 1}")
else:
...
for fold, (train_index, valid_index) in enumerate(skf.split(train_dataset.data, train_dataset.labels)):
   ...
   # Load checkpoint if it's the resuming fold
   if fold == start_fold and start_epoch > 0:
       log_message(f"Loading checkpoint from fold {fold + 1}, epoch {start_epoch}")
   ...
   for epoch in range(epochs):
       ...
       checkpoint_path = f'models/checkpoints/fold{fold+1}_epoch{epoch+1}.pt'
       torch.save(checkpoint, checkpoint_path)

       # Save best model
       if accuracy > best_accuracy:
           best_accuracy = accuracy
           best_model_path = f'models/best_mvt_model_fold{fold+1}.pth'
