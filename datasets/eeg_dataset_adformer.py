...
class VanillaADformerEEGDataset(Dataset):
    ...
    def set_normalization_stats(self, mean, std):
        ...
        self.std = std
        print(
            f"Normalization statistics set externally: mean shape {mean.shape}, std shape {std.shape}"
        )
        ...
        if aug_type == "jitter":
            ...
            if shift > 0:
                aug_data = np.concatenate(
                    [np.zeros_like(aug_data[:shift]), aug_data[:-shift]], axis=0
                )
            elif shift < 0:
                aug_data = np.concatenate(
                    [aug_data[-shift:], np.zeros_like(aug_data[:shift])], axis=0
                )
            ...
            if seg_len > 0:
                for ch_idx in range(data.shape[1]):
                    ...
                    for i in range(segments):
                        if (
                            random.random() < 0.1
                        ):  # 10% chance to permute (reduced from 20%)
                            ...
                            aug_data[start_i:end_i, ch_idx] = aug_data[
                                start_j:end_j, ch_idx
                            ]
            ...
            if mask_len > 0:
                for ch_idx in range(data.shape[1]):
                    if (
                        random.random() < 0.1
                    ):  # 10% chance to apply masking (reduced from 30%)
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            if self.normalize and not np.all(raw_eeg == 0):
                if self.mean is not None and self.std is not None:
                    ...
                    raw_eeg = (raw_eeg - self.mean[:, np.newaxis]) / self.std[
                        :, np.newaxis
                    ]
                else:
        ...
        adformer_formatted_eeg = torch.FloatTensor(raw_eeg).transpose(
            0, 1
        )  # [seq_len, enc_in]
...
def create_vanilla_adformer_dataset(
    data_dir,
    data_info,
    time_length=2000,
    patch_lengths=[200, 400, 800],
...
def create_vanilla_adformer_dataloader(
    dataset, batch_size=32, shuffle=True, sampler=None, num_workers=0
):
    ...
    return DataLoader(
        ...
        ),  # Must be False when using a sampler
        sampler=sampler,
