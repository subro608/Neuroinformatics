...
class SpatialSpectralEEGDataset(Dataset):
    ...
    def __init__(self, data_dir, data_info, scales=[3, 4, 5], time_length=8000, 
        ...
        self.position_matrix = self.position_matrix / np.max(np.abs(self.position_matrix))  # Normalize
    ...
    def compute_spectral_embeddings(self, eeg_data, sfreq=250):
        ...
        for scale in self.scales:
            ...
            combined_embeddings = np.vstack([embeddings] + [lobe_embeddings[lobe] for lobe in self.lobe_groups])
    ...
    def __getitem__(self, idx):
        ...
        try:
            if file_path.endswith('.pkl'):
                ...
                if len(eeg_data) >= 19:
                    if set(self.channel_names).issubset(set(raw.ch_names)):
                        ch_indices = [raw.ch_names.index(ch) for ch in self.channel_names if ch in raw.ch_names]
                ...
                spectral_embeddings = self.compute_spectral_embeddings(eeg_data, sfreq=raw.info['sfreq'])
                for scale_key in spectral_embeddings:
                    spectral_embeddings[scale_key] = torch.FloatTensor(spectral_embeddings[scale_key])
            ...
            for scale in self.scales:
                ...
                if scale_key not in spectral_embeddings:
                    ...
                    if os.path.exists(scale_path):
                        spectral_embeddings[scale_key] = torch.FloatTensor(np.load(scale_path))
                    elif not np.all(raw_eeg == 0):
                        if not spectral_embeddings:
                            spectral_embeddings = self.compute_spectral_embeddings(raw_eeg)
                        spectral_embeddings[scale_key] = torch.FloatTensor(spectral_embeddings[scale_key])
                    else:
                        spectral_embeddings[scale_key] = torch.zeros(19 + len(self.lobe_groups), scale)
            ...
            raw_eeg = np.zeros((19, self.time_length))
            spectral_embeddings = {f'scale_{scale}': torch.zeros(19 + len(self.lobe_groups), scale) 
                                   for scale in self.scales}
            dynamic_adjacency = self.static_adjacency
        
        sample = {
            'raw_eeg': torch.FloatTensor(raw_eeg),
            'adjacency': torch.FloatTensor(dynamic_adjacency if dynamic_adjacency is not None else self.static_adjacency),
...
def create_dataset(data_dir, data_info, scales=[3, 4, 5], time_length=8000, adjacency_type='combined'):
