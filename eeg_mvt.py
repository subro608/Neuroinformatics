...
class MVTEEG(nn.Module):
    def __init__(self, num_channels=19, num_classes=3, dim=128):
        ...
        self.time_conv = nn.Conv1d(num_channels, dim, kernel_size=64, stride=32)
        self.time_encoder = MVTEncoder(dim)
        
        # Frequency domain processing
        self.freq_conv = nn.Conv1d(num_channels, dim, kernel_size=32, stride=16)
        self.freq_encoder = MVTEncoder(dim)
        
        # Spatial processing
        self.spatial_embed = nn.Linear(num_channels, dim)
