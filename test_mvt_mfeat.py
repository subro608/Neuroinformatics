...
from eeg_mvt_dataset_mfeat import EEGDataset
...
if not os.path.exists('images_wavelet'):
   os.makedirs('images_wavelet')
if not os.path.exists('test_results_wavelet'):
   os.makedirs('test_results_wavelet')

# Model parameters (match the updated training parameters)
num_chans = 19
num_classes = 3
dim = 512  # Updated to match the training script
dropout_rate = 0.1

def test_model(data_type='test_cross', include_wavelet=True):
   # Load model from checkpoint
   checkpoint_path = 'models_wavelet/best_mvt_wavelet_model_fold2.pth'
   ...
   print(f'Wavelet features included: {include_wavelet}\n')
   ...
   with torch.no_grad():
       ...
       for inputs_dict, labels in tqdm(test_dataloader, desc="Testing"):
           ...
           with torch.cuda.amp.autocast(device_type=device.type):
               # Pass only the time domain features to match the training script
   ...
   feature_mode = "with_wavelet" if include_wavelet else "without_wavelet"
   results_file = f'test_results_wavelet/results_{data_type}_{feature_mode}.txt'
   with open(results_file, 'w') as f:
       f.write(f'Test Results for {data_type} ({feature_mode})\n\n')
   ...
   plt.ylabel('Actual')
   plt.title(f'Confusion Matrix ({data_type}, {feature_mode})')
   plt.savefig(f'images_wavelet/mvt_confusion_matrix_{data_type}_{feature_mode}.png')
   plt.close()
   ...
   plt.title(f'ROC Curves ({data_type}, {feature_mode})')
   plt.legend(loc="lower right")
   plt.savefig(f'images_wavelet/mvt_roc_curves_{data_type}_{feature_mode}.png')
   plt.close()

   return accuracy, mF1

if __name__ == "__main__":
   # Test both with and without wavelet features for comparison
   print("\n=== Testing with wavelet features ===")
   cross_acc_wave, cross_f1_wave = test_model('test_cross', include_wavelet=True)
   within_acc_wave, within_f1_wave = test_model('test_within', include_wavelet=True)
   
   print("\n=== Testing without wavelet features ===")
   cross_acc_nowave, cross_f1_nowave = test_model('test_cross', include_wavelet=False)
   within_acc_nowave, within_f1_nowave = test_model('test_within', include_wavelet=False)
   
   print("\n=== Comparing Results ===")
   print("With Wavelet Features:")
   print(f"  Cross-subject - Accuracy: {100 * cross_acc_wave:.2f}%, F1: {100 * cross_f1_wave:.2f}%")
   print(f"  Within-subject - Accuracy: {100 * within_acc_wave:.2f}%, F1: {100 * within_f1_wave:.2f}%")
   
   print("\nWithout Wavelet Features:")
   print(f"  Cross-subject - Accuracy: {100 * cross_acc_nowave:.2f}%, F1: {100 * cross_f1_nowave:.2f}%")
   print(f"  Within-subject - Accuracy: {100 * within_acc_nowave:.2f}%, F1: {100 * within_f1_nowave:.2f}%")
   
   # Calculate improvement
   cross_acc_imp = cross_acc_wave - cross_acc_nowave
   cross_f1_imp = cross_f1_wave - cross_f1_nowave
   within_acc_imp = within_acc_wave - within_acc_nowave
   within_f1_imp = within_f1_wave - within_f1_nowave
   
   print("\nImprovement with Wavelet Features:")
   print(f"  Cross-subject - Accuracy: {100 * cross_acc_imp:.2f}%, F1: {100 * cross_f1_imp:.2f}%")
   print(f"  Within-subject - Accuracy: {100 * within_acc_imp:.2f}%, F1: {100 * within_f1_imp:.2f}%")
   
   # Save comparison results
   with open('test_results_wavelet/wavelet_comparison.txt', 'w') as f:
       f.write("=== Comparing Results ===\n")
       f.write("With Wavelet Features:\n")
       f.write(f"  Cross-subject - Accuracy: {100 * cross_acc_wave:.2f}%, F1: {100 * cross_f1_wave:.2f}%\n")
       f.write(f"  Within-subject - Accuracy: {100 * within_acc_wave:.2f}%, F1: {100 * within_f1_wave:.2f}%\n\n")
       
       f.write("Without Wavelet Features:\n")
       f.write(f"  Cross-subject - Accuracy: {100 * cross_acc_nowave:.2f}%, F1: {100 * cross_f1_nowave:.2f}%\n")
       f.write(f"  Within-subject - Accuracy: {100 * within_acc_nowave:.2f}%, F1: {100 * within_f1_nowave:.2f}%\n\n")
       
       f.write("Improvement with Wavelet Features:\n")
       f.write(f"  Cross-subject - Accuracy: {100 * cross_acc_imp:.2f}%, F1: {100 * cross_f1_imp:.2f}%\n")
       f.write(f"  Within-subject - Accuracy: {100 * within_acc_imp:.2f}%, F1: {100 * within_f1_imp:.2f}%\n")
   
   print(f"\nComparison results saved to 'test_results_wavelet/wavelet_comparison.txt'")