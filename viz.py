...
def create_mvt_full_model():
    ...
    components = [
        {'name': 'Raw EEG Input', 'x': 1, 'y': 7, 'width': 2, 'height': 1, 'type': 'input'},
        {'name': 'Spectral Embeddings\nScales 3,4,5', 'x': 1, 'y': 3, 'width': 2, 'height': 1, 'type': 'input'},
        {'name': 'Spatial Feature\nExtractor', 'x': 4, 'y': 7, 'width': 2, 'height': 2, 'type': 'spatial'},
        {'name': 'Multi-Scale Graph\nModule', 'x': 4, 'y': 3, 'width': 2, 'height': 2, 'type': 'spectral'},
        {'name': 'Spatial Features', 'x': 7, 'y': 7, 'width': 1.5, 'height': 1, 'type': 'spatial'},
        {'name': 'Graph Features', 'x': 7, 'y': 3, 'width': 1.5, 'height': 1, 'type': 'spectral'},
        {'name': 'Cross-View\nAttention', 'x': 9.5, 'y': 5, 'width': 1.5, 'height': 2, 'type': 'attention'},
        {'name': 'Fusion', 'x': 12, 'y': 5, 'width': 1.5, 'height': 2, 'type': 'fusion'},
        {'name': 'Classifier', 'x': 14.5, 'y': 5, 'width': 1.5, 'height': 1, 'type': 'output'}
    ]
    ...
    legend_elements = [
        ...
        patches.Patch(facecolor=colors['spatial'], edgecolor='black', label='Spatial Processing'),
        patches.Patch(facecolor=colors['spectral'], edgecolor='black', label='Spectral Processing'),
        patches.Patch(facecolor=colors['attention'], edgecolor='black', label='Cross-Attention'),
        patches.Patch(facecolor=colors['fusion'], edgecolor='black', label='Feature Fusion'),
        patches.Patch(facecolor=colors['output'], edgecolor='black', label='Classification')
    ]
    ax.legend(handles=legend_elements, loc='upper center', bbox_to_anchor=(0.5, 0.05), ncol=3)
    
    # Add a title and labels
    ax.set_title('Multi-View Transformer (MVT) Architecture for Alzheimer\'s Detection', fontsize=16, pad=20)
    ax.set_xlim(0, 17)
    ax.set_ylim(1, 10)
    ax.axis('off')
...
def create_multiscale_graph_visualization():
    ...
    components = [
        # Scale 3 path
        {'name': 'Scale 3 Input\n(B×24×3)', 'x': 1, 'y': 8, 'width': 2, 'height': 1, 'type': 'input'},
        {'name': 'Scale 3\nEmbedding', 'x': 4, 'y': 8, 'width': 2, 'height': 1, 'type': 'embed'},
        {'name': 'Scale 3\nAttention', 'x': 7, 'y': 8, 'width': 2, 'height': 1, 'type': 'attention'},
        
        # Scale 4 path
        {'name': 'Scale 4 Input\n(B×24×4)', 'x': 1, 'y': 5, 'width': 2, 'height': 1, 'type': 'input'},
        {'name': 'Scale 4\nEmbedding', 'x': 4, 'y': 5, 'width': 2, 'height': 1, 'type': 'embed'},
        {'name': 'Scale 4\nAttention', 'x': 7, 'y': 5, 'width': 2, 'height': 1, 'type': 'attention'},
        
        # Scale 5 path
        {'name': 'Scale 5 Input\n(B×24×5)', 'x': 1, 'y': 2, 'width': 2, 'height': 1, 'type': 'input'},
        {'name': 'Scale 5\nEmbedding', 'x': 4, 'y': 2, 'width': 2, 'height': 1, 'type': 'embed'},
        {'name': 'Scale 5\nAttention', 'x': 7, 'y': 2, 'width': 2, 'height': 1, 'type': 'attention'},
        
        # Fusion and Graph components
        {'name': 'Concatenation', 'x': 10, 'y': 5, 'width': 2, 'height': 1, 'type': 'fusion'},
        {'name': 'Dynamic\nAdjacency', 'x': 10, 'y': 3, 'width': 2, 'height': 1, 'type': 'graph'},
        {'name': 'Graph\nConvolution', 'x': 13, 'y': 5, 'width': 2, 'height': 1, 'type': 'graph'},
        {'name': 'Graph Attention\nNetworks', 'x': 16, 'y': 5, 'width': 2, 'height': 1.5, 'type': 'attention'},
        {'name': 'Global Pooling', 'x': 19, 'y': 5, 'width': 2, 'height': 1, 'type': 'fusion'},
        {'name': 'Graph Features', 'x': 22, 'y': 5, 'width': 2, 'height': 1, 'type': 'output'}
    ]
    ...
    freq_bands = [
        ...
        {'x': 1, 'y': 6.2, 'text': 'Scale 4: [(0.5-4Hz), (4-8Hz), (8-13Hz), (13-30Hz)]'},
        {'x': 1, 'y': 3.2, 'text': 'Scale 5: [(0.5-4Hz), (4-8Hz), (8-13Hz), (13-30Hz), (30-50Hz)]'}
    ]
    ...
    legend_elements = [
        patches.Patch(facecolor=colors['input'], edgecolor='black', label='Input'),
        patches.Patch(facecolor=colors['embed'], edgecolor='black', label='Feature Embedding'),
        patches.Patch(facecolor=colors['attention'], edgecolor='black', label='Attention Mechanism'),
        patches.Patch(facecolor=colors['fusion'], edgecolor='black', label='Feature Fusion'),
        patches.Patch(facecolor=colors['graph'], edgecolor='black', label='Graph Operations'),
...
def create_cross_attention_visualization():
    ...
    nodes = {
        ...
        's2g_attention': {'x': 4, 'y': 6.5, 'size': 20, 'label': 'Spatial→Graph\nAttention'},
        'g2s_attention': {'x': 4, 'y': 3.5, 'size': 20, 'label': 'Graph→Spatial\nAttention'},
        
        'spatial_attended1': {'x': 7, 'y': 8, 'size': 25, 'label': 'Spatial Attended'},
        'graph_attended1': {'x': 7, 'y': 2, 'size': 25, 'label': 'Graph Attended'},
        
        's2g_attention2': {'x': 10, 'y': 6.5, 'size': 20, 'label': 'Spatial→Graph\nAttention 2'},
        'g2s_attention2': {'x': 10, 'y': 3.5, 'size': 20, 'label': 'Graph→Spatial\nAttention 2'},
        
        'spatial_attended2': {'x': 13, 'y': 8, 'size': 25, 'label': 'Spatial Attended 2'},
        ...
        'fusion': {'x': 19, 'y': 5, 'size': 30, 'label': 'Feature Fusion\n(Multi-head Attention)'},
